...
struct InstructionIncrement
{
  ...
  {
    if (!jump (false, instruction.jumper, instruction.position, instruction.mask))
      computer_.moveInstructionPointer(instruction.increment ());
  }
...
using Instruction = std::variant <Add, Multiply, Input, Output, JumpIfTrue, JumpIfFalse, Halt>;
...
int Computer::getInstructionPointer () const
{
