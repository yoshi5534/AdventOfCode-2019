...
bool SpaceImage::operator==(SpaceImage const &other) const {
  return (layers_ == other.layers_ && width_ == other.width_ &&
...
int SpaceImage::checksum() {
  ...
  return ranges::count(layers_[minZeroLayer], 1) *
         ranges::count(layers_[minZeroLayer], 2);
}

SpaceImage SpaceImage::fromDigitalSendingNetwork(int width, int height,
