  ...
  struct Tracer
  {
    ...
    void follow (int length, int xDir, int yDir)
    {
      ...
      for (int i=0; i < length; ++i)
        path_.push_back (Location {std::get<0> (location)+(i+1)*xDir, std::get<1> (location) +(i+1)*yDir});
    }

    void operator()(Right const& direction)
    {
      follow (direction.length, 1, 0);
    }
    void operator()(Left const& direction)
    {
      follow (direction.length, -1, 0);
    }
    void operator()(Up const& direction)
    {
      follow (direction.length, 0, 1);
    }
    void operator()(Down const& direction)
    {
      follow (direction.length, 0, -1);
    }
  };
...
Intersections AdventOfCode::intersects (WirePath path1, WirePath path2)
{
  ...
  auto it = std::set_intersection (std::begin(path1), std::end(path1), std::begin(path2), std::end(path2), crossings.begin ());
